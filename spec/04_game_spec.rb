...
describe 'Game' do
  describe '::WIN_COMBINATIONS' do
    it 'defines a constant WIN_COMBINATIONS with arrays for each win combination' do
      expect(Game::WIN_COMBINATIONS).to include([0,1,2])
      expect(Game::WIN_COMBINATIONS).to include([3,4,5])
      expect(Game::WIN_COMBINATIONS).to include([6,7,8])
      expect(Game::WIN_COMBINATIONS).to include([0,3,6])
      expect(Game::WIN_COMBINATIONS).to include([1,4,7])
      expect(Game::WIN_COMBINATIONS).to include([2,5,8])
      expect(Game::WIN_COMBINATIONS).to include([0,4,8])
      expect(Game::WIN_COMBINATIONS).to include([2,4,6])
    end
  end
  ...
  describe 'play' do
    ...
    it 'plays the first turn of the game' do
      ...
      expect(game.board.cells).to eq(["\e[0;91;49mX\e[0m", " ", " ", " ", " ", " ", " ", " ", " "])
    end
    ...
    it 'plays the first few turns of the game' do
      ...
      expect(game.board.cells).to eq(["\e[0;91;49mX\e[0m", "\e[0;91;49mX\e[0m", " ", "\e[0;93;49mO\e[0m", " ", " ", " ", " ", " "])
    end
    ...
    it 'congratulates the winner X' do
      ...
      expect($stdout).to receive(:puts).with("\e[0;91;49mCONGRATULATIONS X\e[0m")
      expect($stdout).to receive(:puts).with("\e[0;93;49m   YOU'VE WON!\e[0m")
    ...
    it 'congratulates the winner O' do
      ...
      expect($stdout).to receive(:puts).with("\e[0;91;49mCONGRATULATIONS O\e[0m")
      expect($stdout).to receive(:puts).with("\e[0;93;49m   YOU'VE WON!\e[0m")
    ...
    it 'prints "Cat\'s Game!" on a draw' do
      ...
      expect($stdout).to receive(:puts).with("\e[0;93;49m   CAT'S GAME!\e[0m")
    ...
    it 'plays through an entire game' do
      ...
      expect($stdout).to receive(:puts).with("\e[0;91;49mCONGRATULATIONS \e[0m\e[0;91;49mX\e[0m")
      expect($stdout).to receive(:puts).with("\e[0;93;49m   YOU'VE WON!\e[0m")
